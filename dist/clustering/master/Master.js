...
class Master extends typed_emitter_1.EventEmitter {
    ...
    constructor(fileName, options) {
        ...
        for (let i = 0; i < keys.length; i++) {
            const key = keys[i];
            this.handlers.on(key, (shard, ...data) => handlers_1.handlers[key]?.bind(shard)(...data));
        }
    }
    debug(msg) {
        this.emit('DEBUG', msg);
    ...
    async start() {
        ...
        const gatewayRequest = await this.rest.get('/gateway/bot');
        ...
        this.chunks = UtilityFunctions_1.chunkShards(this.options?.shards || 1, this.options.shardsPerCluster ?? 5);
